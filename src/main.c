...
/* FMDN discoverable advertisement timeout in seconds */
#define DISC_ADV_TIMEOUT_MS   (2)   // 2 Minute timeout

/* FMDN non discoverable advertisement timeout in seconds */
#define NDISC_ADV_TIMEOUT_ON_MS   (1)   // 1 Minute timeout
#define NDISC_ADV_TIMEOUT_OFF_MS   (9)   // 1 Minute timeout
...
static bool fmdn_enable_flag = false;
static bool fp_adv_clock_sync_done = false;
static bool fp_adv_clock_sync_enabled = false;
...
static void stop_discoverable_adv(struct k_work *work)
{
    LOG_INF("Stopping discoverable advertisement due to timeout\n");
...
static void fmdn_provisioning_state_set(bool provisioned)
{
	...
	fmdn_enable_flag = fmdn_provisioned;
	
	// Stop timeout as provisioning done before timeout
    k_work_cancel_delayable(&stop_discoverable_work);
}
...
static void fmdn_factory_reset_prepare(void)
{
	...
	fp_adv_clock_sync_enabled = false;
...
static void fmdn_advt_mode_change_handle(void)
{
	int err;

	if (!fmdn_provisioned) {
		LOG_INF("FMDN: Can't disable locator functionality without provisioning");
		return;
	}

	bool fast_pair_is_ready = bt_fast_pair_is_ready();

	fmdn_enable_flag = !fmdn_enable_flag;

	if(!fmdn_enable_flag)
	{
		LOG_INF("FMDN: Disable locator functionality");
	...
	{
		LOG_INF("FMDN: Reenable locator functionality");
...
static void fmdn_mode_request_handle(enum app_ui_request request)
{
	...
	if (request == APP_UI_REQUEST_RECOVERY_MODE_ENTER) {
	...
	} else if (request == APP_UI_REQUEST_FMDN_ADV_MODE_CHANGE) {
		fmdn_advt_mode_change_handle();
		...
		if(!fmdn_provisioned)
		{
			...
			if(fp_adv_ui_request==true)
			{
				// Start timeout to stop fast pair discoverable advertisementif not provisioned
				k_work_schedule(&stop_discoverable_work,K_MINUTES(DISC_ADV_TIMEOUT_MS));
				LOG_INF("Start Advt Timeout Timer to disable discoverable advertisement");
			}
			else
			{
				// Stop timeout as provisioning done before timeout
				k_work_cancel_delayable(&stop_discoverable_work);
				LOG_INF("Stop Advt Timeout Timer if already started");
			}
		}
	}
}
