...
#define SPEAKER_BEEP_SHORT_ON_MS		100
#define SPEAKER_BEEP_SHORT_ON_MS2		50
#define SPEAKER_BEEP_SHORT_OFF_MS		500//100
#define SPEAKER_BEEP_SHORT_OFF_MS2		500//100
#define SPEAKER_BEEP_MID_ON_MS			250
#define SPEAKER_BEEP_MID_OFF_MS			500//100
#define SPEAKER_BEEP_LONG_ON_MS			1000
#define SPEAKER_BEEP_LONG_OFF_MS		500//100

/* Thingy LED hardware assignments. */
#define LED_COLOR_RED				DK_LED1
#define LED_COLOR_GREEN				DK_LED1
#define LED_COLOR_BLUE				DK_LED1
...
#define BTN_NOOP_FIRST_START_MS				0
#define BTN_FP_ADV_MODE_CHANGE_START_MS		1000
#define BTN_NOOP_SECOND_START_MS			3000

#define BTN_FMDN_ADV_DIS_ENB_START_MS		10000
#define BTN_NOOP_THIRD_START_MS				15000

#define BTN_ID_MODE_ENTER_START_MS			20000
#define BTN_NOOP_FOURTH_START_MS			25000

#define BTN_RECOVERY_MODE_ENTER_START_MS	30000
#define BTN_NOOP_FIFTH_START_MS				35000

#define BTN_DFU_MODE_ENTER_START_MS			40000
#define BTN_NOOP_SIXTH_START_MS				45000

#define BTN_FACT_RESET_MODE_ENTER_START_MS	50000
#define BTN_NOOP_LAST_START_MS				55000
...
static const struct btn_release btn_release_actions[] = {
	...
	BTN_RELEASE_DEF(APP_UI_REQUEST_COUNT,
			BTN_NOOP_FIFTH_START_MS,
			0,
			SPEAKER_BEEP_DURATION_COUNT),

	BTN_RELEASE_DEF(APP_UI_REQUEST_FACTORY_RESET,
			BTN_FACT_RESET_MODE_ENTER_START_MS,
			5,
			SPEAKER_BEEP_DURATION_SHORT),
...
static void buzzer_beep_handle(struct k_work *w);
static K_WORK_DELAYABLE_DEFINE(buzzer_beep_work, buzzer_beep_handle);
...
static void buzzer_beep_handle(struct k_work *w)
{
    ...
    if (beep_state) {
        /* Buzzer ON */
        app_ui_speaker_on();
        k_work_schedule(&buzzer_beep_work, K_MSEC(SPEAKER_BEEP_SHORT_ON_MS2));
    } else {
        /* Buzzer OFF */
        app_ui_speaker_off();
        k_work_schedule(&buzzer_beep_work, K_MSEC(SPEAKER_BEEP_SHORT_OFF_MS2));
    }
}
	...
	if (has_changed & BTN_MULTI_ACTION) {
		...
		if (button_state & BTN_MULTI_ACTION) {
			...
			if (atomic_test_bit(ui_state_status, APP_UI_STATE_RINGING)) {
				...
				//speaker_beep_play(1, SPEAKER_BEEP_SHORT_ON_MS, SPEAKER_BEEP_SHORT_OFF_MS);
			}
		} else {
			/* On button release */
			int64_t hold_time;
			LOG_ERR("button relase");
			...
			/*if ((btn_idx >= 0) && (btn_idx < ARRAY_SIZE(btn_release_actions)))
			{
				...
				{
					LOG_ERR("request=%d", btn_release_actions[btn_idx].request);
					app_ui_request_broadcast(btn_release_actions[btn_idx].request);
				}
			}*/

			if (prev_uptime != 0) {
				hold_time = (k_uptime_get() - prev_uptime);
				if ((hold_time > BTN_FP_ADV_MODE_CHANGE_START_MS) && (hold_time < BTN_NOOP_SECOND_START_MS)) 
				{
					speaker_beep_play(1, SPEAKER_BEEP_SHORT_ON_MS, SPEAKER_BEEP_SHORT_OFF_MS);
					app_ui_request_broadcast(APP_UI_REQUEST_FP_ADV_MODE_CHANGE);	
				} 
				else if ((hold_time > BTN_FMDN_ADV_DIS_ENB_START_MS) && (hold_time < BTN_NOOP_THIRD_START_MS)) 
				{
					speaker_beep_play(2, SPEAKER_BEEP_SHORT_ON_MS, SPEAKER_BEEP_SHORT_OFF_MS);
					app_ui_request_broadcast(APP_UI_REQUEST_FMDN_ADV_MODE_CHANGE);
				} 
				else if ((hold_time > BTN_ID_MODE_ENTER_START_MS) && (hold_time < BTN_NOOP_FOURTH_START_MS)) 
				{
					speaker_beep_play(3, SPEAKER_BEEP_SHORT_ON_MS, SPEAKER_BEEP_SHORT_OFF_MS);
					app_ui_request_broadcast(APP_UI_REQUEST_ID_MODE_ENTER);
				} 
				/*else if ((hold_time > BTN_RECOVERY_MODE_ENTER_START_MS) && (hold_time < BTN_NOOP_FIFTH_START_MS)) 
				{
					speaker_beep_play(4, SPEAKER_BEEP_SHORT_ON_MS, SPEAKER_BEEP_SHORT_OFF_MS);
					app_ui_request_broadcast(APP_UI_REQUEST_RECOVERY_MODE_ENTER);
				} 
				else if ((hold_time > BTN_DFU_MODE_ENTER_START_MS) && (hold_time < BTN_NOOP_SIXTH_START_MS)) 
				{
					speaker_beep_play(5, SPEAKER_BEEP_SHORT_ON_MS, SPEAKER_BEEP_SHORT_OFF_MS);
					app_ui_request_broadcast(APP_UI_REQUEST_DFU_MODE_ENTER);
				}*/ 
				else if ((hold_time > BTN_FACT_RESET_MODE_ENTER_START_MS) && (hold_time < BTN_NOOP_LAST_START_MS)) 
				{
					speaker_beep_play(6, SPEAKER_BEEP_SHORT_ON_MS, SPEAKER_BEEP_SHORT_OFF_MS);
				...
				{
					//app_ui_request_broadcast(APP_UI_REQUEST_COUNT);
				}
...
int app_ui_state_change_indicate(enum app_ui_state state, bool active)
{
	...
	if (state == APP_UI_STATE_RINGING) {
		...
		if (active && (btn_idx != -1)) {
			...
			if(active)
			{
				k_work_schedule(&buzzer_beep_work, K_NO_WAIT);
			}
			else
			{
				app_ui_speaker_off();
				k_work_cancel_delayable(&buzzer_beep_work);
			}
		}
	}
...
int app_ui_init(void)
{
	...
	//Removed as in field, we will not have a option of power cycle device for factory reset
	//bootup_btn_handle();
